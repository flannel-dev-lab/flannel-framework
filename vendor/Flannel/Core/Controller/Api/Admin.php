...
abstract class Admin extends \Flannel\Controller\Api {
    ...
    protected $_authErrors = [
        1 => 'Unable to authenicate employee',
    ...
    protected function _authenticate() {
        if ($accessToken = \Flannel\Input::server('HTTP_ACCESS_TOKEN')) {
            try {
                ...
                $this->_employee = (new \Model_Employee())->load($data->id);

                // Allow for access tokens to be invalidated
                if ($this->_employee->getAccessKey() != $data->ak) {
                    $this->_addAuthError(3);
                }

                if ($this->_employee->getStatus() == \Model_Employee::STATUS_DISABLED) {
                    $this->_addAuthError(4);
                }   

                if ($this->_employee->getStatus() == \Model_Employee::STATUS_PW_RESET) {
                    $this->_addAuthError(5);
                }   

                if ($this->_employee->getStatus() == \Model_Employee::STATUS_PW_LOCK) {
