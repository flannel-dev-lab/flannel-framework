...
abstract class Merchant extends \Flannel\Controller\Api {
    ...
    protected $_authErrors = [
        1 => 'Unable to authenicate user',
        2 => 'Access token must be provided',
        3 => 'Invalid access token',
        4 => 'This account has been disabled',
        5 => 'Please reset your password',
        6 => 'Your account has been locked due to too many failures',
        7 => 'You must be an owner to access this area',
    ];
    ...
    protected function _authenticate() {
        if ($accessToken = \Flannel\Input::server('HTTP_ACCESS_TOKEN')) {
            try {
                ...
                $this->_user = (new \Model_User())->load($data->u);

                // Allow for access tokens to be invalidated
                if ($this->_user->getAccessKey() != $data->ak) {
                    $this->_addAuthError(3);
                }

                if ($this->_user->getStatus() == \Model_User::STATUS_DISABLED) {
                    $this->_addAuthError(4);
                }   

                if ($this->_user->getStatus() == \Model_User::STATUS_PW_RESET) {
                    $this->_addAuthError(5);
                }   

                if ($this->_user->getStatus() == \Model_User::STATUS_PW_LOCK) {
                    $this->_addAuthError(6);
                }   

                if ($this->_user->getType() != \Model_User::TYPE_OWNER) {
